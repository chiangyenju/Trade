...
def enter_position(df, symbol):
    ...
    try:
        telegram_message = f'{symbol} E: {entry_p} SL: {stop_loss_p} via {strategy_name}'
        send_telegram_message(telegram_token, telegram_chat_id, telegram_message)
...
def check_tp(df, symbol):
    try:
        ...
        if (
            # Condition 1: Close price greater than or equal to the upper Bollinger Band (bb_u)
            (df.loc[df.index[-1], 'Close'] >= df.loc[df.index[-1], 'bb_u']) or
            (
                # Condition 2: Multiple conditions connected by 'and' operators
